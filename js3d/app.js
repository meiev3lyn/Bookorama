// Default original books to be shown in the slider.
const originalBooks = [
  {
...
// We store the original books in LocalStorage.
let storedBooks = JSON.parse(localStorage.getItem('allBooks')) || [];
let books = [];

// Update the books array with the original books and any stored books. We call this function whenever we need to refresh the book list.
...
function deleteBook(bookToDelete) {
  // Deleting a book from the stored books.
  ...
  // Deleting a book from the user's list.
  ...
  // If the book deleted is an original book, we store its image in Localstorage to prevent it from being added again.
...
function renderSlider(booksToShow = books, isSearch = false) {
  ...
  // If there are no books to show in the slider, we show an empty state where the user can add books of their own.
  if (books.length === 0) {
    slider.innerHTML = `
  ...
  // If the user is searching for a book and it is not found, we show a popup asking if they want to add it.
...
function loadShow() {
  ...
  // moving the active books to the right.
  ...
  // moving the active books to the left. 
  stt = 0;
...
//Deleting all books, we ask the user for confirmation before proceeding, we mark all original books as deleted and clear the stored books.
document.getElementById('deleteAllBtn')?.addEventListener('click', () => {
  if (confirm("Are you sure you want to delete ALL books? This cannot be undone.")) {
    const allOriginalImages = originalBooks.map(book => book.image);
    localStorage.setItem('deletedOriginals', JSON.stringify(allOriginalImages));
    localStorage.removeItem('allBooks');
    localStorage.removeItem('myBookList');
    updateBooks();
    renderSlider([]);
    showNotification("All books have been permanently deleted", "success");
  }
});
// We create a modal to show the book details when the user clicks on a book in the slider.
function showModal(book) {
  ...
  addToListBtn.onclick = () => {
    ...
    // When the book is added, we change the button text and disable it.
  ...
  // when the book details are shown, we show the buttons based on whether the book is in the list or not. 
...
// We add an event listener to the search input to filter the books based on the user's input.
const searchInput = document.getElementById('searchInput');
searchInput?.addEventListener('input', () => {
  const query = searchInput.value.toLowerCase().trim();

  // update the books array to include the latest stored books.
  updateBooks();

  // Filtering the books based on the search query.
...
// We add an event listener to the toggle checkbox to show or hide the books in the slider.
...
// Update the books and render the slider when the page loads.
updateBooks();
renderSlider();
